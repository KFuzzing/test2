...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  ...
  if (likely(fault == afl->crash_mode)) {
    ...
    if(afl->k_mode && !new_bits && afl->new_edges_found_idx){ //we find local new edges/paths
      ...
      // Method 1: save all local new seeds
      u8 *queue_fn_local = "";
      queue_fn_local = alloc_printf(
        "%s/queue/local_%llu,src:%06u", afl->out_dir, afl->totalOtherFnameNum, afl->current_entry);
      ...
      // Method 2: only save the local new seeds whose paths are same as the seeds in queue
