...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      // get the number of initial seeds
      ...
      if (afl->k_mode){
        if (afl->normal_mode == 0){
          ...
          if (afl->target_q == NULL){ // first time to come here
            // printf("init local_bitmap_size\n");
            for (u32 idx = 0; idx < afl->queued_items; idx++) {
              struct queue_entry *q = afl->queue_buf[idx];
              if (q->ancestor_seed == q && q->first_havoc == 0){
                q->local_bitmap_size = count_non_255_bytes(afl, q->virgin_bits);
                q->weighted_cov = (double)q->local_bitmap_size;
                // q->bitmap_incre = (double)q->local_bitmap_size;
              }
            }
          }
          
          // 5 epoches -> update
          if (afl->epoch_cnt % 100 == 0){

            u8 keep_running = 0;

            if (afl->target_q){
              // check if we need to update
              struct queue_entry *target_q = afl->target_q;
              u32 tmp = count_non_255_bytes(afl, target_q->virgin_bits);

              if (tmp == afl->last_local_edge_num){ //cannot find new edges, need to re-select the seeds
                
                // printf("\n\n------ epoch_cnt:%u ------\n", afl->epoch_cnt);

                // update current local_bitmap_size
                u32 found_local_new_edges = tmp - target_q->local_bitmap_size;
                target_q->local_bitmap_size = tmp;
                if (target_q->local_bitmap_size > afl->max_local_bitmap_size){
                  afl->max_local_bitmap_size = target_q->local_bitmap_size;
                }

                // update decre_rate
                double tmp_rate = ((double)target_q->local_bitmap_size - (double)afl->min_local_bitmap_size)/((double)afl->max_local_bitmap_size - (double)afl->min_local_bitmap_size);
                target_q->decre_rate = 1.0 / (1.0 + exp(-10 * ((tmp_rate) - 0.5)));
                // if (target_q->decre_rate > 0.98) target_q->decre_rate = 0.98;

                // update weighted_cov
                target_q->cov_w = target_q->decre_rate * target_q->cov_w;
                target_q->weighted_cov = target_q->cov_w * (double)target_q->local_bitmap_size;

                // based on decre_rate, update bitmap_incre
                // update local_bitmap_size and bitmap_incre: only update the previous selected seed
                // printf("==== update local_bitmap_size and bitmap_incre ====\n");
                // printf("update id:%u\n", afl->target_q_entry);
                target_q->bitmap_incre = ((double)found_local_new_edges) + target_q->decre_rate * target_q->bitmap_incre;
                // if (target_q->bitmap_incre < 0.00001) target_q->bitmap_incre = 0.00001;
                // printf("fname:%s weighted_cov:%lf bitmap_incre:%lf\n", target_q->fname, target_q->weighted_cov, target_q->bitmap_incre);

              }else{
                keep_running = 1;
                afl->last_local_edge_num = tmp; 

                target_q->cov_w = 1; // if find new edges, we just reset the weight
                // printf("-- find new edges --\n");
              }
            }

            if (keep_running == 0){
              // get max and min
              afl->max_weighted_cov = 0;
              afl->min_weighted_cov = 100000000;

              afl->max_bitmap_incre = 0;
              afl->min_bitmap_incre = 100000000;
              for (u32 idx = 0; idx < afl->queued_items; idx++) {
                struct queue_entry *q = afl->queue_buf[idx];
                if (q->ancestor_seed == q && q->first_havoc == 0){
                  // printf("fname:%s weighted_cov:%lf bitmap_incre:%lf\n", q->fname, q->weighted_cov, q->bitmap_incre);
                  if (q->bitmap_incre > afl->max_bitmap_incre) afl->max_bitmap_incre = q->bitmap_incre;
                  if (q->bitmap_incre < afl->min_bitmap_incre) afl->min_bitmap_incre = q->bitmap_incre;

                  if (q->weighted_cov > afl->max_weighted_cov) afl->max_weighted_cov = q->weighted_cov;
                  if (q->weighted_cov < afl->min_weighted_cov) afl->min_weighted_cov = q->weighted_cov;

                }
              }
              // printf("afl->max_bitmap_incre:%lf afl->max_weighted_cov:%lf\n", afl->max_bitmap_incre, afl->max_weighted_cov);
              // printf("afl->min_bitmap_incre:%lf afl->min_weighted_cov:%lf\n", afl->min_bitmap_incre, afl->min_weighted_cov);

              // printf("\n==== calculate k_rates ====\n");
              // calculate k_rates
              double biggest_k = -1;
              u32 candidate_same_w[1000];
              u32 max_candidate = 1000;
              u32 candidate_same_w_idx = 0;
              for (u32 idx = 0; idx < afl->queued_items; idx++) {
                struct queue_entry *q = afl->queue_buf[idx];
                if (q->ancestor_seed == q && q->first_havoc == 0){
                  q->k_rate = 0;
                  if (afl->max_weighted_cov != afl->min_weighted_cov){
                    q->k_rate += 0.5 * ((double)q->weighted_cov - (double)afl->min_weighted_cov)/((double)afl->max_weighted_cov - (double)afl->min_weighted_cov);
                  }
                  if (afl->max_bitmap_incre != afl->min_bitmap_incre){
                    q->k_rate += 0.5 * (q->bitmap_incre - afl->min_bitmap_incre)/(afl->max_bitmap_incre - afl->min_bitmap_incre);
                  }
                  
                  if (q->k_rate > biggest_k){
                    candidate_same_w[0] = idx;
                    candidate_same_w_idx = 1;
                    biggest_k = q->k_rate;
                  }else if (q->k_rate == biggest_k){
                    if (candidate_same_w_idx < max_candidate){
                      candidate_same_w[candidate_same_w_idx] = idx;
                      candidate_same_w_idx++;
                    }
                    
                  }

                  // printf("fname:%s k_rate:%lf\n", q->fname, q->k_rate);
                }
              }

              afl->target_q_entry = candidate_same_w[rand() % candidate_same_w_idx];
              afl->target_q = afl->queue_buf[afl->target_q_entry];

              afl->last_local_edge_num = afl->target_q->local_bitmap_size; 
              // printf("target_q_entry:%u target_q:%s\n\n",afl->target_q_entry, afl->target_q->fname);
            }
          }

          // printf("current target_q:%s\n", afl->target_q->fname);
          ...
          for (u32 idx = 0; idx < afl->queued_items; idx++) {
            struct queue_entry *q = afl->queue_buf[idx];
            // if (q->ancestor_seed == q && q->first_havoc && !q->was_fuzzed){
            if (q->ancestor_seed == afl->target_q){
